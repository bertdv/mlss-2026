...
# ╔═╡ d18a2832-fcbf-4887-9622-805e5eabdb26
data(; time, gain) = map(bin_positions) do x
	val = pdf(Normal(0, 1), x)

	rand(Poisson(time * val + gain))
	
end

# ╔═╡ 44f4fe49-70f5-4e25-9218-72133b6aae6c
bar(
	bin_positions,
	data(; time, gain),
	# xlim=(-4,4),
...
# ╔═╡ 1b17279a-436e-48ca-9a4c-99448f6fd416
data

# ╔═╡ 726ac9bb-0538-47e3-b003-ed36cf93af13
@model function poisson_noise(X, α, θ)
	λ ~ Gamma(shape=α, rate=θ)
...
# ╔═╡ 2762673f-7848-43d5-b7bd-878feffd3148
results = infer(
	model=poisson_noise(α=0.001, θ=0.001),
	data=(X=data(; time, gain),),
)

# ╔═╡ f01d4452-d77c-4be5-8c1f-901479006869
join(mean_std(results.posteriors[:λ]), " ± ") |> Text

# ╔═╡ d7dcc9ee-54fd-4dba-be71-900756be6aa4
gain
...
yooo(z) = pdf(Normal(0, 1), z)

# ╔═╡ c38e1211-f07a-49ad-8fe1-149f1fb0bdf2
@model function poisson_peak(x, y, α, θ)
	λ ~ Gamma(shape=α, rate=θ)
	
	for i in eachindex(y)
		y[i] ~ Poisson(λ * yooo(x[i]))
	end
end
...
peak_results = infer(
	model=poisson_peak(α=0.001, θ=0.001, ),
	data=(y=peak_data, x=collect(bin_positions),),
)

# ╔═╡ 466a4eed-60b8-4160-9f7e-e5e3c4df4eaf
peak_results.posteriors[:λ] |> mean_std
...
# ╔═╡ 933e6620-ab5e-4259-ac3d-e410d99d903e
randn()*30 + 30

# ╔═╡ 1c3c97c5-a71f-4ada-9bf0-2dcdf65594aa
begin
	const secret_ccd_gain = 10
	const secret_ccd_exposure = 2e3
	const secret_star_position = [
		17.64192061607603
		11.67676006797155
	]
	const secret_star_σ = 4.583086025100812
end;

# ╔═╡ ce52877c-2ae2-4605-b33b-f65504b5545d
img_data = map(Iterators.product(1:30, 1:40)) do (y,x)
	val = pdf(MvNormal(secret_star_position, secret_star_σ), [x,y])
	rand(Poisson(secret_ccd_gain + secret_ccd_exposure*val))
	# val
end;
...
md"""
# Call met Wouter
Moeilijk om met binned data te werken
...
scatter van fotononen ipv binned data

ster achter zwart gat, het licht wordt gebogen (dus de vorm is anders). Probeeer verschillende verdelingen

melkwegstelsel met verschillende hoeken tov aarde. Fit normal en je krijgt verschillende cov
- Maar de sterposities in een melkwegstelsel is niet een gauss prob proces.
